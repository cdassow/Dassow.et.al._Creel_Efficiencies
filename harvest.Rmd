...
harvestEstimates=charv%>%
  ...
  mutate(harvHa=spp.harvest/(lake.area/2.47))
...
# now I have exploitation rates and as a result harvest estimates too
...
p1=ggplot(harvestEstimates)+theme_classic()+
  geom_boxplot(aes(y=harvestEstimates$harvHa, x=plot.month), fill="grey")+
  geom_hline(yintercept = mean(harvestEstimates$harvHa, na.rm = T), color="red")+
  labs(y="Monthly Harvest per Hectare", x="Month")

# plot of harvest by month - logged
p2=ggplot(harvestEstimates)+theme_classic()+
  geom_boxplot(aes(y=log(harvestEstimates$harvHa), x=plot.month), fill="grey")+
  geom_hline(yintercept = log(mean(harvestEstimates$harvHa)), color="red")+ # setting this manually since the 0 exp rates mess up the logging here
  labs(y="Log(Monthly Harvest per Hectare)", x="Month")
...
harv.surv.a=harvestEstimates%>%
  group_by(survey.seq.no)%>%
  summarise(total.harv=sum(harvHa,na.rm = T))
...
ggplot(ttrExp)+theme_classic()+
  ...
  labs(x="Log(Annual Harvest per Hectare)", y="Density", fill="Scenario")
...
#removing 0s since they can't be logged I'm going to operate under the assumption that if a survey gives a 0 harvest estimate with the full survey, then harvs collecting less data isn't going to change that number. 
...
```{r yearAnalysisExampleLakes, fig.width=8,fig.cap="Comparison of the annual harvest per hectare estimated from the reduced data for a handful of creel surveys. Most data reductions do not result in annual harvest per acre estimates that are much different from the actual data (most are within 1 SD of the harvest estimate for that treatment). Colors represent data reduction scenarios, points are mean annual harvest per hectare estimates across all months of that creel survey and vertical lines represent 1 standard deviation."}
ttrExp=ttrExp%>%
  left_join(cserv[,2:5])%>%
...
ggplot(pdat)+theme_classic()+
  ...
  labs(x="Lake-Year",y="Annual Harvest per Hectare (+/- 1 SD)",color="Scenario")+
  scale_color_viridis_d()

```

```{r year-analysis-diff-actual, fig.cap="Boxplot of the difference between the actual annual harvest per hectare estimate and the data-reduced annual harvest per hectare estimate for each creel survey in the creel dataset. Most of the data exhibits differences very near 0 except the seasonal reductions, dots represent outliers (x<|> x's percentile-1.5*interquartile range)"}
...
kable(exceedSummary,
      ...
      caption = "Counts of the number of times the annual harvest per hectare estimate for the reduced data scenario differs from the actual annual harvest per hectare estimate for that treatment by more than 1 standard deviation. NAs ocurred where there was no variance in the harvest estimate and thus no standard deviation could be calculated for the actual data.")

```

```{r yearAnalysisRecCode, fig.cap="Comparison of the magnitutde of the difference between the mean actual annual harvest per hectare estimate and the estimate derived from each reduced data set. Vertical lines represent 1 standard deviation. Walleye recruitment codes are along the x axis. Note the y-axis scale, many of the differences are small.Walleye recruitment codes describe the source of young-of-year walleye in the system and are defined as follows: c=combined equal shares stocked and natural source, cnr=combined mainly from natural, cst=combined mainly from stocking, nr=natural reproduction only, ost=stocking only with few/no adult walleye present,, st=stocked only, NA=not evaluated yet."}
...
```{r yearAnalysisLakeArea, fig.cap="Comparison of the magnitude of the difference between the mean actual harvest per hectare and the estimate derived from each reduced data set. Note the y-axis scale, many of the differences are small."}
ggplot(trLake_chars)+theme_classic()+
  ...
  labs(x="Log(Lake Area in Hectares)", y="Difference from Actual harvest per ha",color="Scenario")
```

```{r yearAnalysisEffDens, fig.cap="Comparison of the magnitude of the difference between the mean actual harvest per hectare and the estimate derived from each reduced data set. Note the y-axis scale, many of the differences are small."}
ggplot(trLake_chars)+theme_classic()+
  geom_point(aes(y=a.diff,x=log(effortHrs.hectare),color=treat))+
  scale_color_viridis_d()+theme(legend.position = c(0.8,0.4))+
  labs(x="Log(Effort/Hectare)",y="Difference from Actual harvest per ha",color="Scenario")
```

```{r yearAnalysisLakeClass,fig.cap="Comparison of the magnitude of the difference between the mean actual annual harvest per hectare estimate and the estimate derived from each reduced data set. Vertical lines represent 1 standard deviation. Note the y-axis scale, many of the differences are very small.  Lake classes are according to Rypel et al. 2019, https://doi.org/10.1002/fsh.10228 ."}
lcSum=trLake_chars%>%
...
ggplot(lcSum)+theme_classic()+
  ...
  labs(x="Lake Class",y="Mean Difference from Actual Annual Harvest per ha Estimate", color="Scenario")
```
...
ggplot(ttrExp)+theme_classic()+
  ...
  labs(y="Density",x="Log(Annual Harvest per Hectare)", fill="Scenario")
```
...
All comparisons presented in this analysis are comparing a model's ability to fit the data **within** each data set and **not** across data sets which is what is necessary to gain inference on the effect of any data reductions.
...
a.p.gamma=ggplot(aComp.gamma)+theme_classic()+
  ...
  scale_fill_viridis_d()+labs(x="Annual Harvest per Hectare", y="Density", title = "Actual", fill=element_blank())
nw.p.gamma=ggplot(nwComp.gamma)+theme_classic()+
  geom_density(aes(x=harv.hectare,fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Annual Harvest per Hectare", y="Density", title = "No Winter Data \n(April - October)", fill=element_blank())
ma.p.gamma=ggplot(maComp.gamma)+theme_classic()+
  geom_density(aes(x=harv.hectare,fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Annual Harvest per Hectare", y="Density", title = "May - August \nData Only", fill=element_blank())
wd25.p.gamma=ggplot(wd25Comp.gamma)+theme_classic()+
  geom_density(aes(x=harv.hectare,fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Annual Harvest per Hectare", y="Density", title = "25% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
wd50.p.gamma=ggplot(wd25Comp.gamma)+theme_classic()+
  geom_density(aes(x=harv.hectare,fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Annual Harvest per Hectare", y="Density", title = "50% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
we50.p.gamma=ggplot(we50Comp.gamma)+theme_classic()+
  geom_density(aes(x=harv.hectare,fill=treat),alpha=0.2)+
  scale_fill_viridis_d()+labs(x="Annual Harvest per Hectare", y="Density", title = "50% of Weekdend \nCreel Visits/Month \nRemoved", fill=element_blank())
