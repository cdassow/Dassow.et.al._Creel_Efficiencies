...
# now I have exploitation rates and the total effort estimates that come with
...
ceff.ct=ceff%>% # creel effort estimates on a per hectare of water basis
...
```{r monthlyEff,fig.width=8,fig.cap="Distribution of effort estimates by month (panel a) for all years of CTWI creel data. Horizontal line is overall mean effort. Black points are outliers. Panel B is the same information presented on a log-scale. "}
...
p1=ggplot(ceff.ct)+theme_classic()+
  ...
  geom_hline(yintercept = mean(ceff.ct$annual.eff.hectare, na.rm = T), color="black")+
...
p2=ggplot(ceff.ct)+theme_classic()+
  ...
  geom_hline(yintercept = log(mean(ceff.ct$annual.eff.hectare)), color="black")+ # setting this manually since the 0 exp rates mess up the logging here
...
ggplot(ttrExp)+theme_classic()+
  ...
  labs(x="Log(Annual Effort per Hectare)", y="Density", fill="Scenario")
```
...
```{r yearAnalysisExampleLakes, fig.width=8,fig.cap="Comparison of the annual effort per hectare estimated from the reduced data for a handful of creel surveys. Most data reductions do not result in annual effort per hectare estimates that are much different from the actual data (most are within 1 SD of the effort estimate for that treatment). Colors represent data reduction scenarios, points are mean annual effort per hectare estimates across all months of that creel survey and vertical lines represent 1 standard deviation."}
ttrExp=ttrExp%>%
  left_join(cserv[,2:5])%>%
...
kable(exceedSummary,
      ...
      caption = "Counts of the number of times the annual effort per hectare estimate for the reduced data scenario differs from the actual annual effort per hectare estimate for that treatment by more than 1 standard deviation. NAs ocurred where there was no variance in the effort estimate and thus no standard deviation could be calculated for the actual data.")

```

```{r yearNRCode, fig.cap="Comparison of the magnitutde of the difference between the mean actual annual effort per hectare estimate and the estimate derived from each reduced data set. Vertical lines represent 1 standard deviation. Walleye recruitment codes are along the x axis. Note the y-axis scale, many of the differences are small. Walleye recruitment codes describe the source of young-of-year walleye in the system and are defined as follows: c=combined equal shares stocked and natural source, cnr=combined mainly from natural, cst=combined mainly from stocking, nr=natural reproduction only, ost=stocking only with few/no adult walleye present,, st=stocked only, NA=not evaluated yet."}
...
```{r yearAnalysisLakeArea, fig.cap="Comparison of the magnitude of the difference between the mean actual effort per hectare and the estimate derived from each reduced data set. Note the y-axis scale, many of the differences are small."}
ggplot(trLake_chars)+theme_classic()+
  ...
  labs(x="Log(Lake Area in Hectares)", y="Difference from Actual Effort per Hectare",color="Scenario")
```
...
ggplot(ttrExp)+theme_classic()+
  ...
  labs(y="Density",x="Log(Annual Effort per Hectare)", fill="Scenario")
```
...
ln=ggplot(aComp.lnorm)+theme_classic()+
  ...
  scale_fill_viridis_d()+labs(x="Annual Effort per Hectare",title = "Lognormal")+
  theme(legend.position = c(0.75,0.75))
gma=ggplot(aComp.gamma)+theme_classic()+
  geom_density(aes(x=eff.hectare, fill=treat),alpha=0.3)+
  scale_fill_viridis_d()+labs(x="Annual Effort per Hectare",title = "Gamma")+
  theme(legend.position = c(0.75,0.75))
ggarrange(ln,gma)
...
a.p.gamma=ggplot(aComp.gamma)+theme_classic()+
  ...
  scale_fill_viridis_d()+labs(x="Annual Effort per Hectare", y="Density", title = "Actual", fill=element_blank())
nw.p.gamma=ggplot(nwComp.gamma)+theme_classic()+
  geom_density(aes(x=eff.hectare,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Effort per Hectare", y="Density", title = "No Winter Data \n(April - October)", fill=element_blank())
ma.p.gamma=ggplot(maComp.gamma)+theme_classic()+
  geom_density(aes(x=eff.hectare,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Effort per Hectare", y="Density", title = "May - August \nData Only", fill=element_blank())
wd25.p.gamma=ggplot(wd25Comp.gamma)+theme_classic()+
  geom_density(aes(x=eff.hectare,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Effort per Hectare", y="Density", title = "25% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
wd50.p.gamma=ggplot(wd25Comp.gamma)+theme_classic()+
  geom_density(aes(x=eff.hectare,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Effort per Hectare", y="Density", title = "50% of Weekday \nCreel Visits/Month \nRemoved", fill=element_blank())
we50.p.gamma=ggplot(we50Comp.gamma)+theme_classic()+
  geom_density(aes(x=eff.hectare,fill=treat),alpha=0.2)+coord_cartesian(xlim = c(0,250))+
  scale_fill_viridis_d()+labs(x="Annual Effort per Hectare", y="Density", title = "50% of Weekdend \nCreel Visits/Month \nRemoved", fill=element_blank())
...
This means that the gamma distribution not likely to be appropriate for making comparisons between the actual data and the reduced data to understand whether or not creel effort reductions result in significantly different annual angler effort per hectare estimates or not.
...
# not including x2 for this plot since I'm using the calculation differently. Shows kurtosis as the only pval metric that is markedly different from the rest
